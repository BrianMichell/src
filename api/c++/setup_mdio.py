...
def setup_mdio(env, root=None):
    ...
    LDFLAGS_STRING = (
        ...
        f"-lnlohmann_json_schema_validator,"
        f"-ltensorstore_driver_zarr_bzip2_compressor,"
        f"-ltensorstore_driver_zarr_driver,"
        f"-ltensorstore_driver_zarr_spec,"
        f"-ltensorstore_driver_zarr_zlib_compressor,"
        f"-ltensorstore_driver_zarr_zstd_compressor,"
        f"-ltensorstore_driver_zarr_blosc_compressor,"
        f"-ltensorstore_kvstore_gcs_http,"
        f"-ltensorstore_kvstore_gcs_gcs_resource,"
        f"-ltensorstore_kvstore_gcs_validate,"
        f"-ltensorstore_kvstore_gcs_http_gcs_resource,"
        f"-ltensorstore_kvstore_s3,"
        f"-ltensorstore_kvstore_s3_aws_credentials_resource,"
        f"-ltensorstore_kvstore_s3_credentials_default_credential_provider,"
        f"-ltensorstore_kvstore_s3_credentials_environment_credential_provider,"
        f"-ltensorstore_kvstore_s3_credentials_file_credential_provider,"
        f"-ltensorstore_kvstore_s3_credentials_ec2_credential_provider,"
        f"-ltensorstore_kvstore_s3_s3_metadata,"
        f"-ltensorstore_kvstore_s3_s3_resource,"
        f"-ltensorstore_driver_json,"
        f"-ltensorstore_internal_cache_cache_pool_resource,"
        f"-ltensorstore_internal_data_copy_concurrency_resource,"
        f"-ltensorstore_kvstore_file,"
        f"-ltensorstore_internal_file_io_concurrency_resource,"
        f"-ltensorstore_internal_cache_kvs_backed_chunk_cache,"
        f"-labsl,"
        f"-lblosc,"
        f"-ltensorstore,"
        f"-lre2,"
        f"-lriegeli,"
        f"-ltinyxml2_tinyxml2,"
        f"-lcurl,"
        f"-lopenssl,"
        f"--no-whole-archive,"
        f"-lpthread,"
